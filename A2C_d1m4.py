...
class ActorCritic:
    def __init__(self, act_dim_1, act_dim_2, obs_dim, lr_actor, lr_value, gamma):
        ...
        ckpt_state = tf.train.get_checkpoint_state('./d1m4/')
        self.sess = tf.Session()
        if ckpt_state:
            print('restore')
    ...
    def step(self,obs, obs_matrix, lendata):
        ...
        random_chose = False
        if os.path.exists('max_action_d1m4.npy'):
            max_action_d1m4 = list(np.load('max_action_d1m4.npy',allow_pickle=True))
        else:
            max_action_d1m4 = []

        max_action_d1m4.append(np.argmax(self.prob_weights_1.ravel()))
        np.save('max_action_d1m4.npy',max_action_d1m4)
    ...
    def learn(self, last_value_1, done):
        ...
        # if os.path.exists('act_advantage_d1m4.npy'):
        #     act_advantage_dic = np.load('act_advantage_d1m4.npy', allow_pickle=True).item()
        # else:
        act_advantage_dic = []
        for index,item in enumerate(act_1):
            act_advantage_dic.append((item,act_2[index],advantage_1[index], actor_loss))
        # np.save('act_advantage_d1m4', act_advantage_dic)
        ...
        saver = tf.train.Saver()
        save_path = saver.save(self.sess, "d1m4/A2C.ckpt")
        self.memory.reset()
...
def train(notebook_root,dataset_root,ip):
    ...
    if os.path.exists('reward_list_d1m4.npy'):
        ...
        reward_list = list(np.load('./reward_list_d1m4.npy',allow_pickle=True))
    else:
        reward_list = []

    if os.path.exists('reward_list_r_d1m4.npy'):
        print('reward_list_A2C_r_1 exists')
        reward_list_r = list(np.load('./reward_list_r_d1m4.npy',allow_pickle=True))
    else:
        reward_list_r = []
    ...
    if os.path.exists('value_loss_d1m4.npy'):
        ...
        value_loss_list = list(np.load('./value_loss_d1m4.npy',allow_pickle=True))
    else:
        value_loss_list = []
    if os.path.exists('actor_loss_d1m4.npy'):
        print('actor_loss_d1m4 exists')
        actor_loss_list = list(np.load('./actor_loss_d1m4.npy',allow_pickle=True))
    else:
        actor_loss_list = []

    if os.path.exists('suceed_action_d1m4.npy'):
        print('suceed_action exists')
        suceed_action = list(np.load('./suceed_action_d1m4.npy',allow_pickle=True))
    else:
        suceed_action = []
    if os.path.exists('fail_action_d1m4.npy'):
        print('fail_action exists')
        fail_action = list(np.load('./fail_action_d1m4.npy',allow_pickle=True))
    else:
        fail_action = []
