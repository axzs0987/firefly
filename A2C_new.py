...
class ValueNetwork(object):
    ...
    def step(self, obs, reuse):
        with tf.variable_scope(self.name, reuse=reuse):
            h1 = tf.layers.dense(obs, self.dense_dim, activation=tf.nn.tanh,
                                 kernel_initializer=tf.random_normal_initializer(mean=0, stddev=0.3))
...
class ActorCritic:
    def __init__(self, act_dim_1, act_dim_2, obs_dim, dense_dim, lr_actor, lr_value, gamma):
        ...
        ckpt_state = tf.train.get_checkpoint_state('./test4/')
        self.sess = tf.Session()
        if ckpt_state:
            print('restore')
    ...
    def step(self,obs, obs_matrix, lendata):
        ...
        random_chose = False
        if os.path.exists('max_action.npy'):
            max_action = list(np.load('max_action.npy',allow_pickle=True))
        else:
            max_action = []

        max_action.append(np.argmax(self.prob_weights_1.ravel()))
        np.save('max_action.npy',max_action)
        ...
        if np.random.rand(1) < 0.5:
            ...
            while check_res == False and count < 100:
                if self.prob_weights_2[0][action2] == 1:
                    break
                count += 1
        ...
        return action1,self.prob_weights_1,action2,self.prob_weights_2,random_chose # 输入obs，按照策略pai给出相应的执行动作
    ...
    def learn(self, last_value_1, done):
        ...
        if len(obs) == 1:
            ...
            print('check_-1:',obs[-1][-20:])
            print('check_-2:',obs[-2][-20:])
            print('obs same?',(obs[-1]==obs[-2]).all())
        ...
        # if os.path.exists('act_advantage_test_4.npy'):
        #     act_advantage_dic = np.load('act_advantage_test_4.npy', allow_pickle=True).item()
        # else:
        act_advantage_dic = []
        for index,item in enumerate(act_1):
            act_advantage_dic.append((item,act_2[index],advantage_1[index], actor_loss))
        # np.save('act_advantage_test_4', act_advantage_dic)
        ...
        saver = tf.train.Saver()
        save_path = saver.save(self.sess, "test4/A2C.ckpt")
        self.memory.reset()
...
def train(notebook_root,dataset_root,ip):
    ...
    if os.path.exists('reward_list_test_4.npy'):
        print('reward_list_A2C_1 exists')
        reward_list = list(np.load('./reward_list_test_4.npy',allow_pickle=True))
    else:
        reward_list = []

    if os.path.exists('reward_list_r_test_4.npy'):
        print('reward_list_A2C_r_1 exists')
        reward_list_r = list(np.load('./reward_list_r_test_4.npy',allow_pickle=True))
    else:
        reward_list_r = []

    if os.path.exists('./act_reward_test_4.npy'):
        print('exists')
        act_reward = np.load('./act_reward_test_4.npy',allow_pickle=True).item()
    else:
        act_reward = {}

    if os.path.exists('max_reward_test_4.npy'):
        print('max_reward_A2C_1 exists')
        max_reward = list(np.load('./max_reward_test_4.npy',allow_pickle=True))
    else:
        max_reward = []
    if os.path.exists('value_loss_test_4.npy'):
        print('value_loss_test_4 exists')
        value_loss_list = list(np.load('./value_loss_test_4.npy',allow_pickle=True))
    else:
        value_loss_list = []
    if os.path.exists('actor_loss_test_4.npy'):
        print('actor_loss_test_4 exists')
        actor_loss_list = list(np.load('./actor_loss_test_4.npy',allow_pickle=True))
    else:
        actor_loss_list = []

    if os.path.exists('suceed_action_test_4.npy'):
        print('suceed_action exists')
        suceed_action = list(np.load('./suceed_action_test_4.npy',allow_pickle=True))
    else:
        suceed_action = []
    if os.path.exists('fail_action_test_4.npy'):
        print('fail_action exists')
        fail_action = list(np.load('./fail_action_test_4.npy',allow_pickle=True))
    else:
        fail_action = []
    ...
    for i_episode in range(nepisode):
        ...
        temp_act_reward_dic = []
        act_list_1 = np.zeros([10])
        act_list_2 = np.zeros([10])
        if len(s_t) == 1942:
            s_t = np.concatenate((s_t, act_list_1), axis=0)
        if len(s_t) == 1952:
            s_t = np.concatenate((s_t, act_list_2), axis=0)
        ope_count = 0
        chosed_list = []
        while True:
            ...
            while (str(action1)+str(action2) in chosed_list or str(action1)+str(100) in chosed_list) and try_time < 20:
                ...
                if check_res == False:
                    continue
                try_time += 1
            print('prob weight 1:',act_prob1)
            act_prob1 = act_prob1[0][action1]
            print('prob weight 2:', act_prob2)
            act_prob2 = act_prob2[0][action2]
            if str(action1)+str(action2) in chosed_list or len(chosed_list) == 9:
                action1 = 25
                action2 = 0
            ...
            if check_res == False:
                ...
                try:
                    print('action1_:',action1)
                    print('action2_:',action2)
                ...
                act_list_1[ope_count] = action1
                act_list_2[ope_count] = action2
                ope_count += 1
                if len(s_t_plus_1) == 1942:
                    s_t_plus_1 = np.concatenate((s_t_plus_1, act_list_1), axis=0)
                if len(s_t_plus_1) == 1952:
                    s_t_plus_1 = np.concatenate((s_t_plus_1, act_list_2), axis=0)
            ...
            if reward > 0:
                suceed_action.append((notebook_id,act, reward))
                # reward *= 1000
            if reward < 0 and reward != -1:
                fail_action.append((notebook_id,act, reward))
            # if reward == 0:
            #     reward = 0.5
            ...
            if terminal or terminal1:
                ...
                for index,item in enumerate(temp_act_reward_dic):
                    ...
                    if str(action1) not in act_reward.keys():
                        act_reward[str(action1)] = []
                    act_reward[str(action1)].append((item[2],item[1],item[3],item[4],'advantage:'+str(act_advantage_dic[index][2]),'actor_loss:'+str(act_advantage_dic[index][3]),item[5],item[6],item[7],notebook_id))
                value_loss_list.append(value_loss)
                actor_loss_list.append(actor_loss)
                print('Ep: %i' % i_episode, "|Ep_r: %i" % ep_rwd)
                reward_list.append(ep_rwd)
                np.save('reward_list_test_4', reward_list)
                np.save('reward_list_r_test_4', reward_list_r)
                np.save('act_reward_test_4', act_reward)
                np.save('max_reward_test_4', max_reward)
                np.save('suceed_action_test_4', suceed_action)
                np.save('fail_action_test_4', fail_action)
                np.save('value_loss_test_4', value_loss_list)
                np.save('actor_loss_test_4', actor_loss_list)
                break
