...
class ActorCritic:
    def __init__(self, act_dim_1, act_dim_2, obs_dim, dense_dim, lr_actor, lr_value, gamma):
        ...
        ckpt_state = tf.train.get_checkpoint_state('./test5/')
        self.sess = tf.Session()
        if ckpt_state:
            print('restore')
    ...
    def learn(self, last_value_1, done):
        ...
        # if os.path.exists('act_advantage_test_5.npy'):
        #     act_advantage_dic = np.load('act_advantage_test_5.npy', allow_pickle=True).item()
        # else:
        act_advantage_dic = []
        for index,item in enumerate(act_1):
            act_advantage_dic.append((item,act_2[index],advantage_1[index], actor_loss))
        # np.save('act_advantage_test_5', act_advantage_dic)
        ...
        saver = tf.train.Saver()
        save_path = saver.save(self.sess, "test5/A2C.ckpt")
        self.memory.reset()
...
def train(notebook_root,dataset_root,ip):
    ...
    def create_notebook_pool():
        ...
        print('ntoebook_pool:',notebook_pool)
    ...
    if os.path.exists('reward_list_test_5.npy'):
        print('reward_list_A2C_1 exists')
        reward_list = list(np.load('./reward_list_test_5.npy',allow_pickle=True))
    else:
        reward_list = []

    if os.path.exists('reward_list_r_test_5.npy'):
        print('reward_list_A2C_r_1 exists')
        reward_list_r = list(np.load('./reward_list_r_test_5.npy',allow_pickle=True))
    else:
        reward_list_r = []

    if os.path.exists('./act_reward_test_5.npy'):
        print('exists')
        act_reward = np.load('./act_reward_test_5.npy',allow_pickle=True).item()
    else:
        act_reward = {}

    if os.path.exists('max_reward_test_5.npy'):
        print('max_reward_A2C_1 exists')
        max_reward = list(np.load('./max_reward_test_5.npy',allow_pickle=True))
    else:
        max_reward = []
    if os.path.exists('value_loss_test_5.npy'):
        print('value_loss_test_5 exists')
        value_loss_list = list(np.load('./value_loss_test_5.npy',allow_pickle=True))
    else:
        value_loss_list = []
    if os.path.exists('actor_loss_test_5.npy'):
        print('actor_loss_test_5 exists')
        actor_loss_list = list(np.load('./actor_loss_test_5.npy',allow_pickle=True))
    else:
        actor_loss_list = []

    if os.path.exists('suceed_action_test_5.npy'):
        print('suceed_action exists')
        suceed_action = list(np.load('./suceed_action_test_5.npy',allow_pickle=True))
    else:
        suceed_action = []
    if os.path.exists('fail_action_test_5.npy'):
        print('fail_action exists')
        fail_action = list(np.load('./fail_action_test_5.npy',allow_pickle=True))
    else:
        fail_action = []
    ...
    for i_episode in range(nepisode):
        ...
        while True:
            ...
            if terminal or terminal1:
                ...
                np.save('reward_list_test_5', reward_list)
                np.save('reward_list_r_test_5', reward_list_r)
                np.save('act_reward_test_5', act_reward)
                np.save('max_reward_test_5', max_reward)
                np.save('suceed_action_test_5', suceed_action)
                np.save('fail_action_test_5', fail_action)
                np.save('value_loss_test_5', value_loss_list)
                np.save('actor_loss_test_5', actor_loss_list)
                break
