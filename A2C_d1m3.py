...
class ActorNetwork(object):
    def __init__(self, act_1_dim, act_2_dim, name):
...
class ActorCritic:
    def __init__(self, act_dim_1, act_dim_2, obs_dim, lr_actor, lr_value, gamma):
        ...
        self.act_prob_place = tf.placeholder(tf.float32, [None, act_dim_2], name="q_value")

        actor = ActorNetwork(self.act_dim_1, self.act_dim_2,'actor')
        critic = ValueNetwork(self.act_dim_1, self.act_dim_2,'critic1')
        critic1 = ValueNetwork(self.act_dim_1, self.act_dim_2, 'critic2')
        self.memory = Memory()

        self.act_prob = actor.choose_action_1(self.OBS)
        self.act_prob_2 = tf.nn.softmax(self.act_prob_place, name='act_prob_2')
        ...
        ckpt_state = tf.train.get_checkpoint_state('./d1m3/')
        self.sess = tf.Session()
        if ckpt_state:
            print('restore')
    ...
    def step(self,obs, obs_matrix, lendata):
        ...
        # print('obs:', obs)
        print('step_obs_nan:',self.check_nan(obs))
        ...
        random_chose = False
        if os.path.exists('max_action_d1m3.npy'):
            max_action_d1m3 = list(np.load('max_action_d1m3.npy',allow_pickle=True))
        else:
            max_action_d1m3 = []

        max_action_d1m3.append(np.argmax(self.prob_weights_1.ravel()))
        np.save('max_action_d1m3.npy',max_action_d1m3)
        ...
        self.prob_weights_2 = self.sess.run(self.act_prob_2, feed_dict={self.act_prob_place: self.prob_weights_2}) # action,得到每一个action的概率
        ...
        if np.random.rand(1) < 0.5:
            ...
            random_chose = True
            check_res = check_action_by_rule(action1 + 1, action2 + 1, obs_matrix, lendata, column_num=self.act_dim_2)
            count = 0
            while check_res == False and count < 100:
                self.prob_weights_2[0][action2] = 0
                self.prob_weights_2 = self.sess.run(self.act_prob_2,
                                                    feed_dict={self.act_prob_place: self.prob_weights_2})
                action2 = np.random.randint(0, self.act_dim_2)
    ...
    def check_nan(self,w):
        count = 0
        for i in w:
            if type(i).__name__ == 'ndarray':
                for k in i:
                    if np.isnan(k) or k == np.inf or k==-np.inf:
                        # print(count)
                        break
                        return True
            else:
                if np.isnan(i) or i == np.inf or i == -np.inf:
                    # print(count)
                    return True
            count+=1
        return False
    def look_weight(self):

        # with tf.variable_scope("actor", reuse=True):

        for i in range(0,10):
            if i==0:
                w1 = tf.get_default_graph().get_tensor_by_name('actor/dense/kernel:0')
                w1 = self.sess.run(w1)
                # print('actor_w0')
                if self.check_nan(w1):
                    return True
                b1 = tf.get_default_graph().get_tensor_by_name('actor/dense/bias:0')
                b1 = self.sess.run(b1)
                # print('actor_b0')
                if self.check_nan(b1):
                    return True
            else:
                w1 = tf.get_default_graph().get_tensor_by_name('actor/dense_'+str(i)+'/kernel:0')
                w1 = self.sess.run(w1)
                # print('actor_w'+str(i))
                if self.check_nan(w1):
                    return True
                b1 = tf.get_default_graph().get_tensor_by_name('actor/dense_'+str(i)+'/bias:0')
                b1 = self.sess.run(b1)
                # print('actor_b'+str(i))
                if self.check_nan(b1):
                    return True
        ...
        for i in range(0,9):
            if i==0:
                w1 = tf.get_default_graph().get_tensor_by_name('critic1/dense/kernel:0')
                w1 = self.sess.run(w1)
                # print('critic1_w0')
                if self.check_nan(w1):
                    return True
                b1 = tf.get_default_graph().get_tensor_by_name('critic1/dense/bias:0')
                b1 = self.sess.run(b1)
                # print('critic1_b0')
                if self.check_nan(b1):
                    return True
            else:
                w1 = tf.get_default_graph().get_tensor_by_name('critic1/dense_'+str(i)+'/kernel:0')
                w1 = self.sess.run(w1)
                # print('critic1_w'+str(i))
                if self.check_nan(w1):
                    return True
                b1 = tf.get_default_graph().get_tensor_by_name('critic1/dense_'+str(i)+'/bias:0')
                b1 = self.sess.run(b1)
                # print('critic1_b'+str(i))
                if self.check_nan(b1):
                    return True
        ...
        for i in range(0,9):
            if i==0:
                w1 = tf.get_default_graph().get_tensor_by_name('critic2/dense/kernel:0')
                w1 = self.sess.run(w1)
                # print('critic2_w0')
                if self.check_nan(w1):
                    return True
                b1 = tf.get_default_graph().get_tensor_by_name('critic2/dense/bias:0')
                b1 = self.sess.run(b1)
                # print('critic2_b0')
                if self.check_nan(b1):
                    return True
            else:
                w1 = tf.get_default_graph().get_tensor_by_name('critic2/dense_'+str(i)+'/kernel:0')
                w1 = self.sess.run(w1)
                # print('critic2_w'+str(i))
                if self.check_nan(w1):
                    return True
                b1 = tf.get_default_graph().get_tensor_by_name('critic2/dense_'+str(i)+'/bias:0')
                b1 = self.sess.run(b1)
                # print('critic2_b'+str(i))
                if self.check_nan(b1):
                    return True
    ...
    def learn(self, last_value_1, done):
        ...
        print('obs before learn:', self.check_nan(obs))
        ...
        # if os.path.exists('act_advantage_d1m3.npy'):
        #     act_advantage_dic = np.load('act_advantage_d1m3.npy', allow_pickle=True).item()
        # else:
        act_advantage_dic = []
        for index,item in enumerate(act_1):
            act_advantage_dic.append((item,act_2[index],advantage_1[index], actor_loss))
        # np.save('act_advantage_d1m3', act_advantage_dic)
        ...
        saver = tf.train.Saver()
        save_path = saver.save(self.sess, "d1m3/A2C.ckpt")
        self.memory.reset()
...
def check_action_by_rule(action1, action2, s_t, len_data,column_num):
    if action2 != column_num and action2 >= len_data+1:
        return False
    is_column = True
...
def train(notebook_root,dataset_root,ip):
    ...
    agent = ActorCritic(act_dim_1=act_1_dim + 1, act_dim_2=column_num, obs_dim=obs_dim,
                        lr_actor=learning_rate, lr_value=learning_rate, gamma=gamma)


    if os.path.exists('reward_list_d1m3.npy'):
        print('reward_list_A2C_1 exists')
        reward_list = list(np.load('./reward_list_d1m3.npy',allow_pickle=True))
    else:
        reward_list = []

    if os.path.exists('reward_list_r_d1m3.npy'):
        print('reward_list_A2C_r_1 exists')
        reward_list_r = list(np.load('./reward_list_r_d1m3.npy',allow_pickle=True))
    else:
        reward_list_r = []

    if os.path.exists('./act_reward_d1m3.npy'):
        print('exists')
        act_reward = np.load('./act_reward_d1m3.npy',allow_pickle=True).item()
    else:
        act_reward = {}

    if os.path.exists('max_reward_d1m3.npy'):
        print('max_reward_A2C_1 exists')
        max_reward = list(np.load('./max_reward_d1m3.npy',allow_pickle=True))
    else:
        max_reward = []
    if os.path.exists('value_loss_d1m3.npy'):
        print('value_loss_d1m3 exists')
        value_loss_list = list(np.load('./value_loss_d1m3.npy',allow_pickle=True))
    else:
        value_loss_list = []
    if os.path.exists('actor_loss_d1m3.npy'):
        print('actor_loss_d1m3 exists')
        actor_loss_list = list(np.load('./actor_loss_d1m3.npy',allow_pickle=True))
    else:
        actor_loss_list = []

    if os.path.exists('suceed_action_d1m3.npy'):
        print('suceed_action exists')
        suceed_action = list(np.load('./suceed_action_d1m3.npy',allow_pickle=True))
    else:
        suceed_action = []
    if os.path.exists('fail_action_d1m3.npy'):
        print('fail_action exists')
        fail_action = list(np.load('./fail_action_d1m3.npy',allow_pickle=True))
    else:
        fail_action = []
    ...
    for i_episode in range(nepisode):
        ...
        while True:
            ...
            try_time = 0
            while (str(action1)+'+'+str(action2) in chosed_list or str(action1)+'+'+str(100) in chosed_list) and try_time < 20:
                action1, act_prob1, action2, act_prob2, random_chose = agent.step(s_t, s_t_p, len_data)
                check_res = check_action_by_rule(action1 + 1, action2 + 1, s_t_p, len_data, column_num=column_num)
                if check_res == False:
                    continue
            ...
            if terminal or terminal1:
                ...
                print('before learn:',agent.look_weight())
                value_loss, actor_loss, act_advantage_dic = agent.learn(last_value, terminal)
                print('after learn:', agent.look_weight())
                ...
                np.save('reward_list_d1m3', reward_list)
                np.save('reward_list_r_d1m3', reward_list_r)
                np.save('act_reward_d1m3', act_reward)
                np.save('max_reward_d1m3', max_reward)
                np.save('suceed_action_d1m3', suceed_action)
                np.save('fail_action_d1m3', fail_action)
                np.save('value_loss_d1m3', value_loss_list)
                np.save('actor_loss_d1m3', actor_loss_list)
                break
